...
- `dot_config/` — XDG config dir (`~/.config/`), containing:
  - `nvim/` — Neovim config (lazy.nvim; the colorscheme follows the active palette in `dot_config/theme`, see `lua/plugins/colorscheme.lua`; plugin loading skipped when running inside VSCode via `vim.fn.exists("g:vscode")`)
